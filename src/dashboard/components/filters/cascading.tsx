...
const CascadingDropdown: React.FC<CascadingDropdownProps> = ({
  ...
  const [selectedValues, setSelectedValues] = useState<Record<string, string>>(
    isEmpty(currentSelectedValues) ? {} : currentSelectedValues,
  );
  ...
  const renderDropdowns = () => {
    return hierarchy.map((level, index) => {
      const options: Array<DropdownOption> =
        data[level]?.length > 0 ? data[level] : [];
      return (
        <div key={level}>
          <div className="label">
          ...
          <select
            ...
            className="select select-bordered w-60 select-primary"
          >
            <option value="">-- Select --</option>
            {options.length &&
              options.map((option: DropdownOption, optIndex: number) => (
                <option
                  key={`${index + optIndex + ""}-${level}-${option.label}`}
                  value={option.value}
                >
                  {option.label}
                </option>
